...
from src.knowledge.rebuild_concepts import rebuild_concept_pages
from src.rag.okf.convert import convert_xhtml_cache_to_okf
from src.rag.whats_new import refresh_whats_new_file
...
# concept-graph-demo/data/whats-new.json
WHATS_NEW_PATH = Path(__file__).resolve().parents[3] / "data" / "whats-new.json"
...
class CorpusRefreshService:
    ...
    def _run_locked(self, refresh_id: str) -> RefreshResult:
        ...
        try:
            ...
            with stage(refresh_id, "concept_pages", self.tracker):
                try:
                    concept_summary = rebuild_concept_pages(settings=self.settings)
                    details["concept_pages"] = {
                        "generated": concept_summary.generated,
                        "fallback": concept_summary.fallback,
                        "skipped": concept_summary.skipped,
                        "path": concept_summary.path,
                        "model": concept_summary.model,
                    }
                    concept_message = (
                        f" Concept pages rebuilt ({concept_summary.generated}"
                        f", {concept_summary.fallback} extract fallbacks)."
                    )
                except Exception as concept_error:
                    details["concept_pages"] = {"error": str(concept_error)[:300]}
                    concept_message = (
                        " Concept page rebuild failed; left existing generated copy."
                    )

            with stage(refresh_id, "whats_new", self.tracker):
                try:
                    whats_new = refresh_whats_new_file(WHATS_NEW_PATH)
                    details["whats_new"] = {
                        "version": whats_new.version,
                        "source_url": whats_new.source_url,
                        "changed": whats_new.changed,
                        "highlights": whats_new.highlights,
                        "sections": whats_new.sections,
                        "path": str(WHATS_NEW_PATH),
                    }
                    whats_new_message = (
                        f" What's New updated to {whats_new.version}."
                        if whats_new.changed
                        else f" What's New already at {whats_new.version}."
                    )
                except Exception as whats_new_error:
                    details["whats_new"] = {
                        "error": str(whats_new_error)[:300],
                        "path": str(WHATS_NEW_PATH),
                    }
                    whats_new_message = (
                        " What's New discovery failed; left existing file in place."
                    )
