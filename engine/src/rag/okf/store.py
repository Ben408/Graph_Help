...
class OkfConcept:
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "pack_concept_ids": list(meta.get("pack_concept_ids") or []),
        }
...
class OkfStore:
    ...
    def catalog_for_pack_concept(
        ...
        *,
        ...
        """OKF catalog rows whose YAML pack_concept_ids include this pack noun."""
        if not concept_id:
            return []
        items: list[dict[str, Any]] = []
        for item in self._by_id.values():
            stamped = [str(x) for x in (item.get("pack_concept_ids") or []) if x]
            if concept_id not in stamped:
                continue
            if types and str(item.get("type") or "") not in types:
                continue
            items.append(item)
        rank = {"HelpTopic": 0, "Procedure": 1, "HelpSection": 2, "UIScreen": 3}
        items.sort(
            key=lambda item: (
                rank.get(str(item.get("type") or ""), 9),
                str(item.get("title") or ""),
            )
        )
        return items[: max(0, limit)]

    def catalog_matching_url_fragments(
        self,
        fragments: list[str],
        *,
        types: tuple[str, ...] | None = None,
        ...
        """Help catalog rows whose page_url contains any of the path fragments."""
        needles = [frag for frag in fragments if frag]
        if not needles:
            return []
        items: list[dict[str, Any]] = []
        seen: set[str] = set()
        for item in self._by_id.values():
            ...
            if not url or not any(frag in url for frag in needles):
                continue
            ident = str(item.get("concept_id") or "")
            if ident in seen:
                continue
            seen.add(ident)
            items.append(item)
        rank = {"HelpTopic": 0, "HelpSection": 1, "Procedure": 2, "UIScreen": 3}

        def sort_key(item: dict[str, Any]) -> tuple[int, int, str]:
            title = str(item.get("title") or "").lower()
            overview = 0 if ("about" in title or "overview" in title) else 1
            return (overview, rank.get(str(item.get("type") or ""), 9), title)

        items.sort(key=sort_key)
        return items[: max(0, limit)]

    def pack_ids_for_chunk(self, page_url: str, heading_path: str = "") -> list[str]:
        """Prefer the matching procedure/topic stamp; else any ids on the page."""
        ids: list[str] = []
        procedure = self.best_procedure(page_url, heading_path) if heading_path else None
        if procedure and procedure.frontmatter:
            ids.extend(str(x) for x in (procedure.frontmatter.get("pack_concept_ids") or []) if x)
        if not ids:
            for item in self._by_page.get(page_url, []):
                if str(item.get("type") or "") == "HelpTopic":
                    ids.extend(str(x) for x in (item.get("pack_concept_ids") or []) if x)
                    break
        if not ids:
            for item in self._by_page.get(page_url, []):
                ids.extend(str(x) for x in (item.get("pack_concept_ids") or []) if x)
